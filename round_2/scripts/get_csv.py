...
def main():
    ...
    with open(args.sentenceFile, "w") as csvfh,\
      ...
      ncsv_writer.writerow(["node_id","sequence_id","sent_id","system_id","annot_id","lang","mt_label","child_count","children","parent","ucca_label","pos","source","target","is_scene","height","num_tokens","ucca_construction_type", "parent_ucca_construction_type"])
      ...
      for sent in get_sentences(inFile):
        ...
        for node in sorted(sent.ucca_tree.nodes):
          ...
          ucca_construction_type = get_cxn_type(node)
          if node.fparent:
            parent_ucca_construction_type = get_cxn_type(node.fparent)
          else:
            parent_ucca_construction_type = 'Root'
          ...
          ncsv_writer.writerow((node.ID, str(sent.sequence_id), str(sent.sent_id), system_id, annot_id, sent.lang, mt_annot, \
                str(child_count), children, parent, tag, pos, source, target, is_scene, str(height), str(num_tokens), str(ucca_construction_type), str(parent_ucca_construction_type) ))
