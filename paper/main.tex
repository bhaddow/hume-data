...
UCCA scheme \parcite{abend2013universal}, an appealing candidate for semantic analysis,
...
Finally, the differences from HMEANT are discussed 
...
\caption{\label{fig:ucca_example_v2}
  ...
  Edge labels mark UCCA categories, listed  in the table.}
%which are largely ignored in this work
\end{figure}
...
structural node with an atomic label. Here we choose ``Orange'' since it approximately captures the meaning
in this context. The ability to mark structural nodes with atomic labels allows
...
``Adequate''. This is because the root node still has two scenes linked by one linker and
so the relationship between child nodes is maintained. There is an error caused by a wrong
translation of the linker ``After'' which translates as ``by'' and this is
marked as ``Red'', but this error is marked at the point at which it occurs.

\subsection{Composite Score}\label{sec:score}

We proceed to detailing how judgements on the semantic units
of the source are aggregated into a composite score. We start by taking a parsiminous approach
...
using Moses \parcite{Koehn:2007}. We used large training sets extracted from
OPUS \parcite{tiedemann:2009}, and
the data sets released for the WMT14 medical translation task \parcite{bojar-EtAl:2014:W14-33}, to provide between
...
Source sentences are all in English, and their UCCA annotation was performed by four
...
For IAA also for
the Czech, we recruited several further annotators to work on a small number of 
...
that not all nodes in all sentences were annotated. Sometimes,
%this was because
the annotator did not finish a sentence or accidentally missed a node, but in many cases it was also because it was an implicit 
...
There are however a number of other serious consequences on focussing on verbal
...
according to conservative estimates that only considers non-auxilliary instances of ``be''.
\\
\item[Nominative argument structures,] which are ignored by  HMEANT are in fact highly
...
sentences\footnote{Argument structures and linkers are explicitly marked in UCCA. We identified non-auxilliary instances of ``be'' and nouns using the NLTK standard tagger. Nominative argument structures are here defined as Scenes whose main relation is headed by a noun.}.
\end{inparadesc}
...
%Another example where a linker is omitted in translation:
%the source \equo{However, this review\dots}, which was translated as \equo{Diese
%\"Uberpr\"ufung\dots} (lit. \equo{This review}).
...
If only monolingual annotators are available, the HUME evaluation can be performed
...
In HMEANT there are no guidelines as to when to propagate and error. In HUME errors are localised as far as possible to where they occur, by the separation of atomic and structural nodes. This allows us to calculate more acurately the total score of the sentence.
...
a wide range of semantic phenomena. We have shown that it can be reliably annotated in 
