...
\begin{abstract}
  ...
  and tuning of structure-aware\bh{What is ``structure-aware MT''} MT, much in the same way that string-based measures,
...
MEANT \cite{lo2011structured})\bh{Should we just refer to HMEANT, as that is the comparable measure from the MEANT family?}.
...
but unlike them, HUME's scores do not decrease with longer sentences.\bh{We may have to focus
more on qualitative aspects, if we just try to sell the measure on IAA, then the figures
are not stunning.}
...
and use native speakers of the target language in order to annotate this output.\bh{Is it obvious how this was done? What about $n$-$m$ alignments?} Since these annotators have to
...
on the source structure which parts have been correctly translated. A typical display from the HUME annotation tool is
shown in \figref{mttool}, where we see the translation mapped onto UCCA nodes and corresponding source words.

There are two kinds of nodes: a word or basic semantic unit, and a structural component
