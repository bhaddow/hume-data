...
and avoids relying on possibly suboptimal reference translations.
...
defined using a phrase structure parse, and applied human ranking to the resulting segments.\oa{is this description OK?}
...
multi-word expressions that translate as one unit. \oamod{For instance,
...
\oamod{One source sentence and one translation are presented at a time.}
...
\oa{This entire section needs reviewing}
...
in terms of their SRL annotations. The discussion is acoompanied by examples from our
experimental data.
...
Conversely, HMEANT does not penalize errors (e.g., partial semantic compatability,
or negation flip) in a correctly aligned predicate:
...
While the translation is mostly understandable, the verb {\"u}berspringen is incorrect here
(should be ``auslassen''). HUME allows marking the main verb as Orange, or even red, while
giving partial score for the translation of ``don't'' and of ``meals''. HMEANT may only
mark the predicate as correct, or mark the entire argument structure as an error.

It is also relatively common that verbal constructions are trasnlated
