...
While this may be interpreted is a minor error, HMEANT will assign it a very low score,
...
the structures.\oa{I didn't understand this}
...
nodes, and consider those nodes which have two annotations from different
annotators.
...
structural and lexical nodes. The numbers are the node counts in each bin. }
\label{fig:iaalength}
\end{figure*}
...
is quite similar (0.6-0.7) for all languages, showing good agreement, but there are
differences observed when we break down by node type.  In particular, we see a contrast  between
Czech and Polish (where the IAA is higher for lexical nodes than structural nodes) and German and Romanian
(where the reverse  is true). We also observe low IAA (around 0.3) in some cases, for instance 
in German lexical nodes, and for Polish and Czech structural nodes.

\XXX{Discussion of the disagreements}

\begin{compactitem}
  \item German has low overall proportion of orange annotation (1.2\% compared with 4.5-8.6)
  \item German annotators give a lot of green (so skewed classes, high expected agreement), but 
        differ more significantly on what should be green. 
  \item For Polish, big difference in number of As (2000 vs 1500) given by each annotator
  \item For Czech structural, both annotators give higher proportion of As, raising the expected agreement.
\end{compactitem}
