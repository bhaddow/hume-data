...
Human judgement is the central (and perhaps the ultimate) criterion for the quality of an MT system.
Nevertheless, common measures for MT evaluation, such as adequacy and frequency judgements or the relative
ranking of several possible translations, are problematic in two ways.
First, as the quality of translation is determined by multiple factors, it is difficult
to assign a single number so to reflect the quality of the entire sentence, even for a
trained annotator. This also shows up with the diminishing IAA scores as a function of the
sentence length \cite{}.
Second, a single number is not instructive enough for localizing the translation errors, so to inform
the improvement of the MT system. This problem is partly addressed by using scores that decompose over
the sub-parts of the sentence, such as BLEU, thereby localizing the translation error.
However, decomposition into n-grams are most suitable for evaluating shallow methods,
such as phrase-based translation, 
and are less instructive for more structurally-aware methods,
which are at the forefront of MT research today.

These difficulties have motivated the definition of a human measure that decomposes over the semantic
structure of the sentence, rather than its n-grams. The most notable attempt of this brand is the HMEANT
measure \cite{LoWu2010,LoWu2011a,LoWu2011b,LoWu2012}, which is based on the similarity
between the translation output and a reference, defined in terms of the verbal argument structures of the
two sentences. While appealing in addressing the two major shortcomings detailed above, HMEANT 
is unable to capture semantic differences which do not show up in the verb-argument structure,
including several very frequent phenomena, such as inter-clause linkage, nominalizations and copula clauses
(see \secref{}). For instance, consider the sentence ``A coronary angioplasty may not be technically possible'',
...
Another problem with HMEANT is that it requires semantically annotating MT output, which is often unintelligible,
as well as involves the annotation and comparison to a reference translation: an impractical if not unattainable
solution, given the huge number of reasonable translations a sentence may have \cite{dryer_marcu:12}.

In this work we propose the Human UCCA-based MT Evaluation ({\it HUME}) metric,
a human evaluation measure that decomposes over local semantic structures.
The measure is based on a decomposition of the source sentence into semantic units according
to the UCCA semantic annotation, and the estimation of their translation quality.
UCCA covers a wide range of fundamental linguistic phenomena,
including verbal, nominal and adjectival argument structures, inter-clause relations, coorindation
and others. Importantly, HUME only requires an annotation of the source sentence, thus avoiding the 
annotatation machine-generated text. By basing the measure on a direct comparison
of the source and translation we avoid the difficulties engendered by using
reference translations, but impose the additional requirement
that the evaluator be proficient in both the source
and target languages.
Further discussion of the strengths and weaknesses of HUME relative to previous approaches
can be found in \secref{}. 
