...
%\let\thefootnote\relax\footnotetext{*All authors contributed equally to this work.}
...
%out of the huge number of possible translations a sentence may have
...
\perscite{machacek:bojar:segranks:2015} divided the source sentences into shorter segments,
defined using a phrase structure parse, and applied human ranking to the resulting segments.
...
%E.g., ``after he graduated, Tom moved to America'' contains two verbs,
%but is similar in meaning to the one-verbed
%``after graduation, Tom moved to America''. 
...
%to America'' as the source $s$, see \figref{fig:ucca_example}.
...
%Orange (as it corresponds to ``will move'' in $t$), and ``America''
...
it particularly suitable for semantic  MT evaluation. HUME is evaluated in terms of its
consistency (inter-annotator agreement), efficiency (time of annotation) and validity (by comparing it
...
\def\iaafig #1{\includegraphics[width=4cm]{iaa_heatmap_#1.png}}
...
%different node types for both
%  the en-de and en-ro experiments.
...
\begin{figure*}[t]
\begin{tabular}{c}
\includegraphics[width=1\textwidth]{tree_653_ver2.png}\\
...
\caption{An example of a doubly annotated HUME en-de sentence. The color of the nodes correspond to their
  HUME label: yellow (A), blue (B), and atomic nodes with their traffic. Nodes in which the annotators
  disagree are split in color. The scale at the bottom right shows a sample of 10 raw DA assessments
  for this example. }
\label{fig:humeeg}
\end{figure*}
%Original sentence ID = 653
%HUME SID=287
%RAW DA
%82.0+36.0+57.0+32.0+74.0+57.0+24.0+95.0+30.0+0.0+70.0+38.0+41.0+92.0+45.0+15.0+59.0+82.0+24.0+87.0+24.0+24.0+86.0+87.0+8.0+36.0+43.0+61.0+41.0+14.0+0.0+15.0+50.0+16.0+26.0+23.0+83.0+39.0+45.0+1.0+80.0+31.0+53.0+12.0+64.0+22.0+46.0+39.0
%STD DA
%-0.345604136518,-3.33742540772,0.423428930981,-0.548184035933,-0.677176346874,-1.65622648693,-1.55926607798,-1.1272965301,-0.0171653191891,-1.02340816821,-0.872524664699,-0.966670725408,-0.280564543666,-3.14230662916,-0.0830045886964,-1.47697478054,0.470426332641,-1.1678743283,-0.432290259946,-1.92671695959,0.141372817859,-1.36373703007,-0.179200976557,-0.427598369803
%
%54.8 = sample of 10 raw scores
%-0.230 = sample of 10 std scores
%Generate a random sample each time: here is one random sample from score_segs.R: 
%57+64+0+39+83+50+45+53+32+46
%57+64+0+39+83+50+45+53+32+46
%36+64+80+41+1+83+46+50+39+95=535 -> show this one as is closest to 54.8



%\subsection{Discussion of Disagreements}\label{sec:disagreements}

While a quantitative comparison between DA and HUME, as we have just presented, is telling of their
differences and commonalities, it is important to note that the two measures are fundamentally different.
...
While DA standardises and averages scores across annotators to denoise the raw data, and obtain
a single aggregate score, HUME decomposes over a combinatorial structure, thus allowing
to localize the translation errors.
...
%Moreover, as the above example shows, a verbal argument structure
%may be translated to a nominal or adjectival argument structure,
%leading to an unduly low HMEANT score.
