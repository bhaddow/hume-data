...
accuracy ratings, direct assessment (DA). By crowdsourcing a large number of adequacy judgements for each candidate 
...
It has been shown that mean DA judgements correlate very strongly with each other. 
We followed~\parcite{graham2015accurate} and collected up to 15 adequacy judgements on most of the
...
We can see in Figure~\ref{fig:dacorrelation} that HUME and Direct Assesment scores correlate reasonably well.  
The Pearsons correlation for en-de is 0.58 and for en-ro is 0.70 overall which confirms that 
the HUME metric is consistent with an accepted human evaluation method, namely DA, 
even though the two metrics are very different in their conception. 
Although DA is a valuable tool for translation research, HUME has two advantages:
it returns fine-grained semantic information about the quality of translations and it only requires one 
annotator. Direct assesment returns a single score, and it needs a large 
crowd which may not be available or reliable. 

\begin{figure}[t]
\includegraphics[width=0.45\textwidth]{humevsDAcorrtypes-10en-dero.pdf}
\caption{Pearsons correlation HUME scores vs DA for different node types. Struct refers to all structural nodes, P+S are all process and state nodes, H parallel scene nodes, A participant, C centre, E elaborators, and L linker nodes.}
\label{fig:dacorrelationtypes}
\end{figure}

In Figure~\ref{fig:dacorrelationtypes} we can see an analysis of HUME correlations with DA by HUME node type. We can see that correlation increases when combining different sematic node types and that correlation is highest for node type ``all''. Some types of nodes seem to predict the DA scores better than others. The HUME scores on parallel scene nodes (H) and participant nodes (A) seem to correlate more strongly with DA than scores on process and state nodes (P+S) or linker nodes (L). Center nodes (C) are also more correlated than elaborator nodes (E), which is what we would expect given that center nodes are by definition more important to the semantics of a sentence than elaborators. 
