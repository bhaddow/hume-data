...
\begin{abstract}
  ...
  Recent interest in semantics-aware MT systems underscores te importance of
  ...
  such as BLEU, support advances with shallower MT systems.  
  ...
  the measure, as well as its reliability, reflected in the inter-annotator agreement
  scores.
...
to assign a single number that quantifies the quality of the whole entire sentence. This
...
translation. More recent work on MT evaluation proposed measures
...
The paper concludes by outlining ongoing work to construct semi- and fully-automatic variants of HUME.
...
Human evaluation is the standard measure, and is
...
indiviaul adequacy/fluency scores to each sentence, a procedure recently improved
...
Scalability is addressed by scores that decompose over the sub-parts of
...
over n-grams or even individual words, e.g., 
...
in the output of shallow MT methods, and are less telling for identifying the semantic
...
To address this shortcoming, more more recent work quantifies
the similarity of the reference and translation output in terms
...
\newcite{gimenez2007linguistic} proposed an evaluation measure that incorapotes features
...
METEOR measures, in not requiring a reference. Instead,
it compares the source directly with the output
...
argument structure, and are thus less motivated as a basis for a semantic measure.
...
we used native speakers of the target language in order to annotate this output. Since these annotators have to
...
There are two kinds of components (nodes): a word or basic semantic unit, and a structural component which contains one
or more sub-components. 

\paragraph{Lexical nodes} are usually comprised of individual words. They are the leaf nodes of the tree, the smallest 
meaning-bearing
units.
