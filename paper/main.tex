...
\author{
  Alexandra Birch\textsuperscript{1}, Omri Abend\textsuperscript{2}, Ond{\v{r}}ej Bojar\textsuperscript{3},
...
different type of metric to HUME. It returns just one score per sentence and requires annotators to make 
decisions collating many smaller judgements about individual errors in the sentence. 
Comparing DA and HUME is important in order to see where they agree and differ in order to
determine under what circumstances it would be appropriate to apply each method. 
In cases where we specifically want to know about the quality of semantic
components in a translation, HUME and HMEANT could be considered.
...
%This is not entirely true - they do allow an alignment with partial
%Conversely, HMEANT does not penalize errors (e.g., incorrect tense or negation flip) in a correctly aligned predicate:
%
%\begin{center}
%\begin{tabular}{lp{5.4cm}}
...
%\end{tabular}
%\end{center}
%While the translation is mostly understandable, the verb {\"u}berspringen is incorrectly inflected. HUME allows marking the %main verb as Orange, while
%marking the translation of the argument (``Malhzeiten'' or ``meals'') as correct. HMEANT may only mark the predicate as %correct, or mark the entire argument structure as an error.
