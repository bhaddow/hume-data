...
%as a function of
with
the sentence length \parcite{Bojar:2011}.
...
HMEANT \parcite{lo2011structured}.
% SAVING SPACE:
...
This allows the re-use of the source semantic annotation for measuring the quality
...
We also avoid creating and relying on possibly suboptimal
...
We also report on the correlation scores of HUME with direct assesment of human 
adequacy ratings. 
Finally, the differences with HMEANT are discussed 
...
e.g., in the WMT tasks \parcite{bojar2015findings}, or by giving
adequacy/fluency scores to each translation, a procedure recently improved
by \perscite{graham2015improving}.
...
\parcite{Dixon:10a,Dixon:10b,Dixon:12}.
...
\parcite{sulem2015conceptual}. This indicates that the scheme reflects
...
The Abstract Meaning Representation (AMR) scheme \parcite{banarescu2013abstract}
...
fine-grained distinctions they provide, and so we argue they are less suitable as a basis
...
Note that UCCA annotation is performed once for every source sentence,
...
The translation is not UCCA-annotated.
...
The basic strategy for annotating HUME is to traverse the semantic units
...
% XXX let's leave the reviewers to notice this:
%  \bh{Note that in this case we also 
...
it approximately captures the meaning
in this context. The ability to mark non-leaves with atomic labels allows
...
and atomic nodes, we are able to define where an error occurs, and localise
...
\begin{figure*}[t]
    ...
    \caption{The HUME annotation tool, based on the UCCA tool. The faded orange box at the top
      contains the translation, and directly below it is the source sentence. The tree of semantic units
      is shown below the source. Alignments between the source and translation appear in italics and unaligned intervening words appear in red.}
    \label{fig:interface}
    \end{center}
\end{figure*}
...
of the source are aggregated into a composite score. We start by taking a parsimonious approach
...
to $t$. Let $\setsize{\Units(s)}$ be the total number of units marked with any of the five labels.
...
Where $a(u)$ is discontinuous in the text, words between the left
...
the whole translation. The meaning of the expression is not preserved in the
translation
and the annotator should mark this structural node as ``B'', bad. 
%The interleaving 
...
accurate translation is paramount here, underscoring the importance of reliable
...
and Polish and one main annotator for Czech.
%In order to allow us to calculate
For IAA also for
Czech, several further annotators worked on a small number of 
...
\tabref{tab:annot}
shows the total number of sentences and nodes annotated by each annotator. Note
that not all nodes in all sentences were annotated, often
due to
%this was because
...
\tabref{tab:iaa} reports the number of nodes which have two annotations from
...
nodes (annotated as Red, Orange or Green) and structural nodes (annotated
as Adequate or Bad).
As expected and confirmed by confusion matrices in \figref{fig:heatmap}, there
is generally little confusion between the two types of nodes.
...
\end{tabular}
\caption{Plots of HUME scores vs Direct Assesment scores which have been standardised for each crowdsourcing annotator and averaged for 10 annotators. HUME scores are averaged where there were two annotations. 
%The regression is calculated using linear least squares.
}
\label{fig:dacorrelation}
\end{figure}
Recent research~\cite{graham2015accurate,graham2015crowd} has proposed a new approach for collecting 
accuracy ratings. By crowdsourcing a large number of adequacy judgements for each candidate 
translation on a fine-grained 
scale of 0 to 100, we can apply statistical measures which result in reliable aggregate scores. 
It has been shown that mean  judgements correlate very strongly with each other. 
We followed~\citep{graham2015accurate} and collected up to 15 adequacy judgements on most of the
...
according to the method discussed in ~\cite{TODO}. 

We can see in Figure~\ref{fig:dacorrelation} that HUME and Direct Assesment scores do correlate. 
The Pearsons correlation for en-de is 0.58 and for en-ro is 0.70 which confirms that 
...
We argue that although annotating HUME requires more expertise and effort than direct assesment it has
two advantages. It returns fine-grained information about the translations and it can use one 
expert annotator. Direct assesment returns only one aggregate score, and requires a large number
of crowdsourcing workers who may not be available or reliable. 

\begin{figure}[t]
\includegraphics[width=0.45\textwidth]{humevsDAcorrtypes-10en-dero.pdf}
\caption{Pearsons correlation HUME scores vs DA for different node types. Struct is correlation for all structural nodes, P+S is correlation for all process and state nodes, H are parallel scene nodes, A participant, C centre, E elaborators, and L linker nodes.}
\label{fig:dacorrelationtypes}
\end{figure}

In Figure~\ref{fig:dacorrelationtypes} we can see an analysis of HUME correlations with DA by HUME node type. Generally correlations are good. Some types of nodes seem to predict the DA scores better than others. The HUME scores on parallel scene nodes (H) and participant nodes (A) seem to correlate more strongly with DA than scores on process and state nodes (P+S) or linker nodes (L). 
...
but the problems are also seen in English.
...
and it is often impossible to assign a semantic structure to a low quality MT output sentence.
...
by the separation of atomic and structural nodes (See Fig \ref{fig:hume_tree_v2} 
for an example).
