...
\begin{tikzpicture}[semithick, x=2.4cm,y=0.8cm, >=latex, baseline=7ex,
  ...
  \node (ROOT) [label={\#7}] [fill=black, circle] {}
  child {node [label={\#1}] {After} edge from parent node[left]  {$L$\hspace{1.5cm}}}
...
\begin{tikzpicture}[semithick, x=2.4cm,y=0.8cm, >=latex, baseline=7ex,
      inner sep=.3ex, outer sep=.7ex, minimum size=1.2ex,->]
  \node (ROOT) [label={G}] [fill=green, circle] {}
  child {node [label={R}] {Golf} edge from parent node[left]  {$L$}}
  child {node [label={\#1}] {became} edge from parent node[left]  {$L$}}
  child {node [label={\#1}] {a} edge from parent node[left]  {$L$}}
  child {node [label={\#1}] {passion} edge from parent node[left]  {$L$}}
  child {node [label={\#1}] {for} edge from parent node[left]  {$L$}}
  child {node [label={\#1}] {his} edge from parent node[left]  {$L$}}
  child {node [label={\#1}] {oldest} edge from parent node[left]  {$L$}}
  child {node [label={\#1}] {daughter} edge from parent node[left]  {$L$}}
  child {node [label={\#1}] {,} edge from parent node[left]  {$L$}}
  child {node [label={\#1}] {she} edge from parent node[left]  {$L$}}
  child {node [label={\#1}] {attended} edge from parent node[left]  {$L$}}
  child {node [label={\#1}] {daily} edge from parent node[left]  {$L$}}
  child {node [label={\#1}] {lesssons} edge from parent node[left]  {$L$}}
  child {node [label={\#1}] {and} edge from parent node[left]  {$L$}}
  child {node [label={\#1}] {greatly} edge from parent node[left]  {$L$}}
  child {node [label={\#1}] {improved} edge from parent node[left]  {$L$}};
...
UCCA's current inventory of distinctions focuses on annotating argument structures
(adjectival, nominal, verbal and others) and relations between them.
The most basic notion is the {\sc scene}, which describes a movement, an
...
latter. Further categories account for
...
(2) human judgements as to the translation quality of each semantic unit of $s$,
defined by its UCCA annotation, relative to $t$.
...
a unit is {\it atomic}, i.e., that it has no meaning-bearing
sub-units, or {\it structural}. In most cases, atomic units 
...
%``Golf became an obsession for his oldest daughter, she took daily lessons and became very good'',
...
individually.\oa{I'm not sure I agree with the UCCA annotation;
also, translation annotation for ``she'' is missing. and for ``very good'' is wrong}
...
\figref{fig:interface} presents the user interface of HUME. The sentence at the top
shows the complete MT system output. The source sentence appears at the top, and
the source translation directly below it. Underneath the source sentence appears
...
The interface presents, for each unit, the segment of the translation which aligns to it.
This allows the user, especially in long sentences, to focus her attention on the parts
mostly likely to be relevant for her judgement. As the alignments are automatically derived,
...
In the event that $a(u)$ is discontiguous in the text, we mark words between the left
and right boundaries of $a(u)$ which are not contained in it (intervening words)
in a smaller red font. Intervening words are likely change the meaning of the translation
...
%See \secref{sec:hmeant_comp} for a more elaborate discussion of the differences
%between HUME
%and HMEANT.
% This is here!
